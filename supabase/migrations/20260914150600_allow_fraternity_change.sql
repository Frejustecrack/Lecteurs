-- ============================================================================
-- PR5 — Changement de fraternité ouvert à TOUS les rôles
--
-- Cahier des charges : l'appartenance à une fraternité est une information de
-- vie de groupe, pas une donnée administrative. Les Caissiers et Responsables
-- doivent pouvoir la corriger sans passer par l'Admin.
--
-- La RLS d'origine (lecteurs_update) réservait tout UPDATE à l'Admin et au CO.
-- Deux mécanismes complémentaires sont mis en place :
--
--   a) public.changer_fraternite(uuid, uuid) — fonction SECURITY DEFINER :
--      point d'entrée privilégié (utilisé par rpc() côté application). Elle
--      valide la cible, journalise l'action et contourne la RLS proprement.
--
--   b) policy lecteurs_update_fraternite + trigger check_lecteur_update :
--      filet de sécurité pour un UPDATE direct (fallback applicatif). La
--      politique ouvre l'UPDATE, le trigger vérifie colonne par colonne qu'un
--      rôle non Admin/CO n'a touché QU'À fraternite_id.
--
